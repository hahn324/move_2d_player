...
fn set_jump(
    ...
    if let Ok((mut animation_indices, mut animation_timer, mut sprite)) = query.single_mut() {
        animation_indices.first = JUMP_SPRITE_INDICES.0;
...
fn set_sprite_crouch(
    ...
    if let Ok((mut animation_indices, mut animation_timer, mut sprite)) = query.single_mut() {
        animation_indices.first = CROUCH_SPRITE_INDICES.1;
...
fn set_sprite_crouch_walk(
    ...
    if let Ok((mut animation_indices, mut animation_timer, mut sprite)) = query.single_mut() {
        animation_indices.first = CROUCH_WALK_SPRITE_INDICES.0;
    ...
    if let Ok((mut controller, gravity_scale)) = query.single_mut() {
        ...
        match player_state.get() {
            ...
            PlayerState::CrouchWalk => match player_direction.get() {
                ...
                PlayerDirection::Left => movement.x -= PLAYER_CROUCH_SPEED,
            },
            PlayerState::Jump => {
            ...
            PlayerState::JumpFall => match player_drift.get() {
                PlayerDrift::Right => movement.x += PLAYER_DRIFT,
                PlayerDrift::Left => movement.x -= PLAYER_DRIFT,
                PlayerDrift::None => (),
            },
